...
class EnTam(Dataset, Logger):
    ...
    def __init__(self, l1_fpath, l2_fpath, start_stop=True, verbose=True, cache_id=0, vocabularies=None, word2vecs=None,
        ...
        self.pad_idx_1 = self.l1_vocab.sorted_tokens.index(self.reserved_tokens[1])
        self.pad_idx_2 = self.l2_vocab.sorted_tokens.index(self.reserved_tokens[1])
        self.bos_idx = self.l2_vocab.sorted_tokens.index(self.reserved_tokens[4])
        self.eos_idx = self.l2_vocab.sorted_tokens.index(self.reserved_tokens[5])
    ...
    def __getitem__(self, idx):
        ...
        E_mask = [1 if x != self.pad_idx_1 else 0 for x in E]
        #T_mask = [1 if x != self.pad_idx_2 else 0 for x in T]

        return np.array(E), np.array(E_mask), np.array(T)
